            ...
            <tbody class="divide-y divide-gray-200 bg-white" v-if="items.length > 0">
              ...
              <tr>
                  <template v-for="(elm, i) in headers" :key="i">
                    <td class="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900" v-if="(typeof(elm.show) === 'undefined' || elm?.show) && elm?.key !== 'action'">
                      {{item[elm.key]}}
                    </td>
      ...
      <div>
        <nav class="isolate inline-flex -space-x-px rounded-md shadow-sm" aria-label="Pagination">
          ...
          <div v-for="(item, index) in links" :key="index">
            <div v-if="item.label == fulldata.current_page" class="relative h-10 inline-flex items-center border border-indigo-500 bg-indigo-50 px-4 py-2 text-sm font-medium text-indigo-600 z-10">{{item.label}}</div>
            <div @click="getData(item.url)" v-else-if="item.label == 'pagination.previous'" href="#" class="relative h-10 inline-flex items-center border border-gray-300 bg-white px-2 py-2 text-sm font-medium text-gray-500 hover:bg-gray-50 focus:z-20  cursor-pointer">
              <span class="sr-only">First</span>
              <Icon name="chevron-left" class="h-4 w-4" />
            </div>
            <div @click="getData(item.url)" v-else-if="item.label == 'pagination.next'" href="#" class="relative h-10 inline-flex items-center border border-gray-300 bg-white px-2 py-2 text-sm font-medium text-gray-500 hover:bg-gray-50 focus:z-20 cursor-pointer">
