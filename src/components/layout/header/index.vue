<template>
	<header
		class="sticky top-0 bg-primary shadow border-b-8 border-secondary z-30"
	>
				...
				<div class="flex-1 px-1 flex justify-end" ref="dropdown">
					<div class="ml-4 flex items-center md:ml-6">
						<div class="ml-3 relative">
							<div
								class="max-w-xs h-6 w-6 bg-primary-light text-white flex items-center justify-center text-xs rounded-full"
							>
								JD
							</div>
						</div>
						<div
							class="text-sm text-white pl-2 cursor-pointer"
							@click="isShow = !isShow"
						>
							{{ user?.name }}
						</div>
						<div class="ml-1 relative">
							<div>
								<button
									type="button"
									class="max-w-xs flex text-white items-center text-sm"
									id="user-menu-button"
									aria-expanded="false"
									aria-haspopup="true"
								>
