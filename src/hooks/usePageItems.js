...
/**
 * use page items hook
 * @param {import("vue").Ref<number>} currentPaginationNumber
 * @param {import("vue").ComputedRef<boolean>} isMultipleSelectable
 * @param {import("vue").ComputedRef<boolean>} isServerSideMode
 * @param {import("vue").Ref<Array>} items
 * @param {import("vue").Ref<number>} rowsPerPageRef
 * @param {import("vue").WritableComputedRef<Array>} selectItemsComputed
 * @param {import("vue").Ref<boolean} showIndex
 * @param {import("vue").ComputedRef<Array>} totalItems
 * @param {import("vue").ComputedRef<number>} totalItemsLength
 */
export default function usePageItems(
	...
	const multipleSelectStatus = computed(() => {
		...
		const isNoneSelected = selectItemsComputed.value.every(
			(itemSelected) => {
				if (
		...
		if (selectItemsComputed.value.length === totalItems.value.length) {
			const isAllSelected = selectItemsComputed.value.every(
				(itemSelected) => {
					if (
						totalItems.value.findIndex(
							(item) =>
