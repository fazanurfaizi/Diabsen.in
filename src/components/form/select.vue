<template>    
    <select 
        ...
        <template v-if="typeof options === 'string'">            
            <option v-for="(option, index) in options" :key="`option-${index}`" :value="option" :id="`option-${index}`">
                {{ option }}
            </option>        
        </template>
        ...
        <template v-if="typeof options !== 'string' && options.length > 0">                    
            <option v-for="(option, index) in options" :key="`option-${index}`" :value="optionValue ? option[optionValue] : option" :id="`option-${index}`">
