...
import { defineComponent, reactive, toRefs, onMounted, onUpdated, watch, onUnmounted } from "vue";
	...
	setup(props, { emit, slots }) {
		...
		const scrollToModelValue = (modelValue, animate = true) => {
			...
			if(option && state.tabPanelsRef) {
				...
				window.requestAnimationFrame(() => checkScrollEnd(state.tabPanelsRef, option.left), () => {
					const tabPanelsRef = (state.tabPanelsRef?.children || [])
					const removeTabPanelRefs = [...tabPanelsRef].filter((_, i) => i !== index)
