<template>
	<button
		...
		:class="[pagination.active && 'bg-indigo-100 text-indigo-600']"
	...
	const paginationItemsForRender = computed(() => {
		...
		if (props.maxPaginationNumber <= props.totalVisible) {
			...
			[
				1,
				2,
				props.maxPaginationNumber,
				props.maxPaginationNumber - 1,
			].includes(props.currentPaginationNumber)
		) {
			// x,x,x,...,x,x,x
			for (let i = 1; i <= props.totalVisible; i += 1) {
				if (i <= 3) {
					...
					const page =
						props.maxPaginationNumber - (props.totalVisible - i)
					paginationItems.push({
						type: 'button',
						page,
			...
			[
				props.maxPaginationNumber - 2,
				props.maxPaginationNumber - 3,
			].includes(props.currentPaginationNumber)
		) {
			// x,...,x,x,x,x,x
			for (let i = 1; i <= props.totalVisible; i += 1) {
